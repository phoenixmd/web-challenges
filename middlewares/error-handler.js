...
module.exports = ({ showStackTrace = false }) => {
  return (err, req, res, next) => {
    const statusCode = err.status || err.statusCode || res.statusCode || HTTPStatus.INTERNAL_SERVER_ERROR;
    const trace = showStackTrace && err.stack ? err.stack : undefined;

    statusCode === HTTPStatus.INTERNAL_SERVER_ERROR ? Logger.error(err.stack || err) : Logger.warn(err.message);
